...
interface directly.  Matrix-free CG instead holds one SPD operator fixed and
uses curvature--vector products to solve each action system; a residual or
energy argument must certify the returned widths.  Replacing the operator by a
window, subsample, or sketch is a separate approximation and requires its own
operator comparison.  The controlled experiment in
