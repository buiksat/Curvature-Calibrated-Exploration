...
The argument is easiest to read as a sequence of interfaces.  A statistical
model supplies confidence in a frozen, predictable metric.  Geometry transports
that confidence to the current relinearized metric.  An operator certificate
then relates the current metric to the SPD operator used for scoring, and a
solver certificate relates exact operator widths to the numerical widths seen
by the action selector.  This section states that composition abstractly;
\Cref{sec:corrected-center} supplies the primary squared-loss confidence model.
...
\paragraph{How one round uses the interfaces.}
The prefix history determines $\bVbar_t$, $\bV_t$, and the representation used
to form the current queries.  Before observing the reward, the learner
constructs $\Calg_t$, obtains its path, operator, and solver certificates,
forms one upper-valid width map over the score domain, and selects an action.
After the reward arrives, it updates the frozen rank-one statistics.  This
order is why confidence can use collection-time queries even though the score
uses current queries.  It also prevents a selected-action solver refinement
from changing the theorem's score without rechecking the selection guarantee.
...
The theorem separates validity from usefulness.  Reference confidence and the
all-domain upper width make the score optimistic.  The reverse metric and
operator comparisons, together with played-action sharpness, return the
selected width to the frozen potential and produce the closed regret bound.
Large $\bar D_t$, a small $\kappa_{-,t}$, loose solver sharpness, persistent
bias, or oracle error can still make that bound uninformative.  No optimizer
residual, CG-specific formula, or dynamic-width complexity appears in the
headline result.  When a lower operator factor is unavailable, the one-sided
dynamic theorem in \Cref{app:one-sided-fallback} remains the applicable fallback.
...
Algorithm~\ref{alg:transported-ucb} specifies interfaces rather than a single
linear algebra implementation.  Exact Cholesky solves satisfy the solver
interface directly in exact arithmetic.  Matrix-free CG instead holds one SPD
operator fixed and uses curvature--vector products to solve each action system;
a residual or energy argument must certify the returned widths.  Replacing the
operator by a window, subsample, or sketch is a separate approximation and
requires its own operator comparison.  The controlled experiment in
\Cref{sec:transport-instantiation} uses dense exact-current matrices and
...
We next instantiate the reference-confidence interface.  The corrected center
keeps statistical estimation in the predictable collection-time features while
using the current nonlinear prediction as its anchor.  The resulting identity
does not assume that the representation parameter minimizes a nonlinear loss.
Let $\sigma>0$ and let
