...
Let $(\mathsf A,\mathcal B)$ be a measurable action space.  At round $t$, the
learner observes a context and a nonempty measurable action domain
$\cA_t\subseteq\mathsf A$.  Write $\cHist_t^-$ for the sigma-algebra before
fresh round-$t$ algorithmic randomization, and
$\cHist_t=\cHist_t^-\vee\sigma(\Omega_t)$ after that randomization but before
the reward.  All quantities used to select the action are
$\cHist_t$-measurable.  We assume the action-indexed scores have measurable,
finite suprema and that the score oracle below returns a measurable action.
This is automatic for finite actions with a deterministic tie rule; a bare
measurable action space alone does not guarantee a measurable selector.
...
\begin{equation}\label{eq:three-widths-new}
 \bar s_t^2(a)=\bq_t(a)^\top\bVbar_t^{-1}\bq_t(a),\qquad
 s_t^2(a)=\bq_t(a)^\top\bV_t^{-1}\bq_t(a),\qquad
...
For predictable $\beta_t\ge0$, center $m_t(a)$, and action-dependent envelope
$b_t(a)\ge0$, a simultaneous confidence event satisfies
...
$\bVbar_t$ to $\bV_t$ with a predictable certificate
...
\end{equation}
Finally, the solver returns $\hat s_t(a)\ge0$ such that
\begin{equation}\label{eq:solver-interface-new}
 s_{\Calg,t}(a)\le\hat s_t(a)\quad\text{for every }a\in\cA_t,
 \qquad
 \hat s_t(a_t)\le\alpha_t s_{\Calg,t}(a_t),\quad\alpha_t\ge1.
\end{equation}
...
\begin{theorem}[Corrected-center, estimator- and solver-agnostic transport]
\label{thm:transported-ucb}
Suppose \Cref{asm:abstract-transport} holds, the score obeys
...
\begin{corollary}[Finite full enumeration]
\label{cor:finite-enumeration-new}
If $\cA_t$ is finite and the learner maximizes \eqref{eq:transported-score-new}
with a deterministic tie rule, then \eqref{eq:approx-score-oracle-new} holds
with $\xi_t=0$, and all score-supremum and selector measurability conditions in
\Cref{thm:transported-ucb} are automatic.
\end{corollary}
...
\begin{algorithm}[t]
\caption{Corrected-center transported UCB}\label{alg:transported-ucb}
\begin{algorithmic}[1]
\STATE Observe the context and measurable action domain $\cA_t$.
\STATE Choose any predictable representation parameter $\btheta_t$ in the certified region.
\STATE Form the query map $\bq_t(a)$, corrected center $m_t^{\rm corr}(a)$, and bias envelope $b_t(a)$.
\STATE Construct $\Calg_t$ and obtain certificates $\bar D_t$, $\kappa_{-,t}$, and $\kappa_{+,t}$.
\STATE Obtain solver upper widths $\hat s_t(a)$ over the score domain.
\STATE Return an $\xi_t$-approximate maximizer of the score in \eqref{eq:transported-score-new}.
...
\end{equation}
Equivalently, if
$\dot\Bmat(\tau)^\top\dot\Bmat(\tau)\preceq
\nu_t(\tau)^2\bV(\tau)$ almost everywhere, then
$\mathcal D(\bV)\le2\int_0^1\nu_t(\tau)d\tau$.
\end{lemma}
...
relinearized curvature.
Predictably weighted or heteroscedastic squared loss uses row
...
the reward is revealed.  Fix before data collection a reference parameter
$\btheta^\circ\in\Theta$ with $\|\btheta^\circ\|_2\le S$, and allow
...
\begin{table}[t]
\centering\small
\begin{tabular}{p{.25\linewidth}p{.31\linewidth}p{.18\linewidth}cc}
\toprule
...
prediction conversion, predictable collection-time Fisher weights, control of
