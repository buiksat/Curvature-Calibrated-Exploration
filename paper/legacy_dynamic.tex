...
The headline theorem closes regret through the frozen potential by using both
sides of the operator comparison.  This appendix preserves the earlier route
for cases where only an upper comparison is available.  That route transfers
optimism, but it leaves a realized dynamic-width complexity in the bound.  It
is a conditional fallback rather than a second headline guarantee.

The fallback uses a finite action enumeration and a Gaussian squared-loss
model.  Its original-center policy needs an optimizer-centered discrepancy
certificate; the corrected-center variant removes that requirement at the cost
of frozen-feature access.  The curvature definitions extend algorithmically to
exponential-family likelihoods, but \Cref{app:expfam} does not claim the
corresponding statistical confidence or regret theorem.

\paragraph{Protocol.}
Let $(\mathcal X,\mathcal B_{\mathcal X})$ be a measurable context space, let
$(\mathcal A,\mathcal B)$ be a standard Borel action space, and fix a
deterministic $A_{\max}\in\mathbb N$.  Before any round-$t$ randomization,
the learner observes a context $\bx_t\in\mathcal X$, a positive
$\cH_t^-$-measurable integer $K_t\le A_{\max}$, and
$\cH_t^-$-measurable candidates
$a_{t,1},\ldots,a_{t,A_{\max}}\in\mathcal A$.  Unused slots may repeat
$a_{t,1}$, and
\[
 \cA_t=\{a_{t,k}:1\le k\le K_t\}.
\]
Assume $(x,a)\mapsto\mu^*(x,a)$ is
$\mathcal B_{\mathcal X}\otimes\mathcal B$-measurable.  The learner selects
...
Define the finite comparator by
\[
 k_t^*:=\min\operatorname*{arg\,max}_{1\le k\le K_t}
       \mu^*(\bx_t,a_{t,k}),\qquad
 a_t^*:=a_{t,k_t^*},\qquad
 \mu_t^*:=\mu^*(\bx_t,a_t^*).
\]
The finite true-mean vector, its maximum, and its smallest maximizing index are
$\cH_t^-$-measurable.  Thus $a_t^*$, $\mu_t^*$, and
$R_T=\sum_{t=1}^T[\mu_t^*-\mu^*(\bx_t,a_t)]$ are random variables once the
measurable learner rule below is fixed.
...
The action-scaled centering certificate (\Cref{asm:optim}) controls the
action-projected discrepancy between $\btheta_t$ and the auxiliary
frozen-feature ridge estimator $\hat\btheta_t^{\mathrm{lin}}$.  It does
not certify convergence to a global minimizer of the nonlinear objective.
...
$a_{t,1},\ldots,a_{t,K_t}$; set $\cA_t=\{a_{t,k}:k\le K_t\}$.
...
\ENDFOR
\STATE Set $k_t$ to the smallest index maximizing
$\widehat\mu_t(a_{t,k})+\omega_t\sqrt{u_t(a_{t,k})/(1-\bar\varepsilon_t)}
\sqrt{\tilde s_t^2(a_{t,k})}$; play $a_t=a_{t,k_t}$. \label{line:ucb}
...
(\Cref{alg:ccucb}, with full enumeration of the $K_t$ active candidate indices)
under the
...
suppose the measurably enumerated finite-domain protocol above holds and
\Cref{alg:ccucb} enumerates every active index using its smallest-index score
rule, a predictable schedule $\bar\beta_t\ge\beta_t$, and uniform per-round CG
tolerance.  The finite score vector is pre-reward measurable; its maximum and
smallest maximizing index are measurable.  Hence the selected action, realized
design, and regret are well-defined random variables.  Let
