...
deterministic $A_{\max}\in\mathbb N$.  Before any
round-$t$ randomization, the learner observes a context $\bx_t\in\mathcal X$,
an $\cH_t^-$-measurable integer $K_t\in\{1,\ldots,A_{\max}\}$, and
...
Define the measurable finite comparator by
\[
 ...
 \mu_t^*:=\mu^*(\bx_t,a_t^*).
\]
The finite maximum and its smallest maximizing index are
$\cH_t^-$-measurable.  The goal is to minimize
$R_T=\sum_{t=1}^T[\mu_t^*-\mu^*(\bx_t,a_t)]$.
...
\Cref{alg:ccucb} enumerates all active indices using its smallest-index score
